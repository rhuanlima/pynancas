...
df_accounts.info()
df_accounts["Saldo atual"] = df_accounts["Saldo atual"].apply(
    lambda x: x.replace("BRL", "")
)
df_accounts["Saldo atual"] = df_accounts["Saldo atual"].apply(
    lambda x: x.replace(".", "")
)
...
#%%
df_transactions = df[df["Nome"].isnull() == True][
    [
        "Conta",
        "Transferências",
        "Descrição",
...
#%%
df_transactions["dt_transaction"] = pd.to_datetime(
    df_transactions["Data"] + " " + df_transactions["Tempo"]
)

del df_transactions["Tempo"]
del df_transactions["Data"]
df_transactions["Valor"] = df_transactions["Valor"].apply(
    lambda x: x.replace("BRL", "")
)
...
df_transactions = pd.merge(
    df_transactions,
    df_accounts[["id_account", "des_account"]],
    on="des_account",
    how="left",
)
...
df_transactions = pd.merge(
    df_transactions,
    df_accounts[["id_account", "des_account"]],
    left_on="Transferências",
    right_on="des_account",
    how="left",
)
...
#%%
dfa = df_transactions.groupby(["id_account_from", "des_account_from"]).agg(
    {"balance": "sum", "dt_transaction": "min"}
)
#%%
dfb = df_accounts.groupby(["id_account"])["vlr_full_balance"].sum().reset_index()
df_transactions_accounts = pd.merge(
    dfa, dfb, left_on="id_account_from", right_on="id_account", how="left"
)
df_transactions_accounts["open_balance"] = (
    df_transactions_accounts["vlr_full_balance"] - df_transactions_accounts["balance"]
)
df_transactions_accounts.info()
df_transactions_accounts.rename(columns={"id_account": "id_account_from"}, inplace=True)
df_transactions_accounts.rename(
    columns={"des_account": "des_account_from"}, inplace=True
)
...
df_transactions["id_transaction"] = df_transactions.apply(
    lambda x: make_transaction(x["id_account_from"], x["id_account_to"], x["balance"]),
    axis=1,
)


df_transactions["des_info_bol"] = df_transactions["des_info"].isnull()
df_transactions["des_info"] = df_transactions.apply(
    lambda x: x["des_info"] if not x["des_info_bol"] else "", axis=1
)
#%%

df_transactions.to_csv("data/export/transactions.csv", index=False)
df_accounts.to_csv("data/export/accounts.csv", index=False)
df_transactions_accounts.to_csv("data/export/transactions_accounts.csv", index=False)
#%%
df_transactions_accounts
#%%
...
df_analise["bol_check"] = df_analise["des_info"].apply(
    lambda x: x.upper().find(" DE ") != -1
)
df_analise[df_analise["bol_check"] == True]

df_transactions_accounts

df_transactions.groupby(["id_account_from"])["balance"].sum()
